...
class SDMDBH{
	...
	private function __construct(){
		...
		$config = new DBConfig();
		$dns = $config->getDriver().":";
		$dns .= "host=".$config->getDBHost().";";
		if(!empty($port)){
			$dns .= "port=".$config->getDBPort().";";
		}
		$dns .= "dbname=".$config->getDBName();

		try {
			$this->pdo = new PDO($dns, $config->getDBUserName(), $config->getDBUserPassword(), $this->db_opts);
	...
	public static function getInstance($handler){
		$instance = null;
		/*
		 $instanceArray = self::$instanceArray;
		if(!isset($instanceArray[$handler]) || ($instanceArray[$handler] == null))
		{
		$instance = new SDMDBH(DBConfigFactory::getConfig($handler));
		self::$instanceArray[$handler] = $instance;
		} else {
		$instance = self::$instanceArray[$handler];
		}
		return $instance;
		*/
		// 		$instance = self::$instanceArray[$handler];
	 ...
	 * @return string
	 */
	public function execute_stored_procedure($query, $params=null, $format = "json"){

		if($this->checkObj()){
			try {
				if(isset($params)){
						
					// 					if($params instanceof SDMDBParameters)
						// 						{
						// 							print "success";
						// 						}else{
						// 							print "fail";
						// 						}
							...
							// 							( get_class($params) == "Database\DBHandler\SDMDBParameters" || get_class($params) == "SDMDBParameters")		){
									$params instanceof SDMDBParameters){
		...
		return $format=="json"?$this->genResultJson():$this->genResultArray();
	}
	 ...
	 * @return string
	 */
	public function execute_sql_statement($query, $format = "json"){
		if($this->checkObj()){
			try {
	...
	private function genResultArray(){
		$jsonArray["response"]["system"]["errorNo"] = $this->m_result;
		$jsonArray["response"]["system"]["errorMsg"] = $this->m_result_message;
		$jsonArray["response"]["system"]["rowCount"] = $this->m_result_count;
		if(isset($this->m_result_response)){
